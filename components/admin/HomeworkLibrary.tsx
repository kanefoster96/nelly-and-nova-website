...
function DrillEditorView({
  ...
  return (
    <div className="mt-8">
      ...
      <DrillBlockEditor
        blocks={drill.blocks}
        onAdd={(block) => addBlock(pillarId, categoryId, drill.id, block)}
        onText={(blockId, text) => updateBlockText(pillarId, categoryId, drill.id, blockId, text)}
        onMove={(blockId, dir) => moveBlock(pillarId, categoryId, drill.id, blockId, dir)}
        onRemove={(blockId) => removeBlock(pillarId, categoryId, drill.id, blockId)}
      />
    </div>
  );
}
